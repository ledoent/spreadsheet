...
"""
Tests for Feature 6 — Headless XLSX Export (spreadsheet_xlsx_export).
"""
import base64
import io
...
class TestXlsxExport(TransactionCase):
            ...
            "sheets": [
                {
                    ...
                    "name": "Summary",
                    "cells": {
                        "0": {"0": {"content": "Revenue"}, "1": {"content": "12345"}},
                        "1": {"0": {"content": "Cost"},    "1": {"content": "7000"}},
                    },
                }
            ]
        }
    ...
    def test_render_multiple_static_sheets(self):
        raw = {
            "sheets": [
                {"id": "s1", "name": "Sheet1", "cells": {"0": {"0": {"content": "A"}}}},
                {"id": "s2", "name": "Sheet2", "cells": {"0": {"0": {"content": "B"}}}},
            ]
        }
        self.spreadsheet.write({"spreadsheet_raw": raw})
        wb = self._load_wb(SpreadsheetXlsxExporter(self.env, self.spreadsheet).render())
    ...
    def test_render_with_unknown_pivot_model_graceful(self):
        """Unknown model in pivot definition should not crash; writes error cell."""
        raw = {
        ...
        # Should not raise
        wb = self._load_wb(SpreadsheetXlsxExporter(self.env, self.spreadsheet).render())
        # Pivot sheet should exist with an error message
        self.assertIn("Bad Pivot", wb.sheetnames)
        ws = wb["Bad Pivot"]
        self.assertIn("Unknown model", ws.cell(1, 1).value)

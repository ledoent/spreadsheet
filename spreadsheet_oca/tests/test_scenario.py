...
"""
Tests for spreadsheet.scenario (Feature 5 — Named Scenarios / What-If Manager).
"""
import json
...
class TestSpreadsheetScenario(TransactionCase):
            ...
            "sheets": [
                {
                    ...
                    "name": "Sheet1",
                    "cells": {
                        "2": {  # row 3 (0-based index 2)
                            "1": {"content": "50000"},  # col B (0-based index 1)
                        }
                    },
                },
                {
                    ...
                    "name": "Assumptions",
                    "cells": {
                        "4": {  # row 5 (0-based index 4)
                            "2": {"content": "0.08"},  # col C (0-based index 2)
                        }
                    },
                },
            ],
    ...
    def test_apply_to_copy_writes_cell(self):
        ...
        cell_content = new_raw["sheets"][0]["cells"]["2"]["1"]["content"]
    ...
    def test_apply_to_copy_with_sheet_prefix(self):
        ...
        cell_content = new_raw["sheets"][1]["cells"]["4"]["2"]["content"]
    ...
    def test_apply_does_not_modify_original(self):
        ...
        original_cell = (
            self.spreadsheet.spreadsheet_raw["sheets"][0]["cells"]["2"]["1"]["content"]
        )
        scenario = self._make_scenario(
            name="Change B3",
    ...
    def test_apply_to_copy_creates_cell_if_missing(self):
        ...
        new_sheet = self.env["spreadsheet.spreadsheet"].browse(new_id)
        new_raw = new_sheet.spreadsheet_raw
        # Z=col25 (0-based), row99 → row_idx=98
        cell_content = new_raw["sheets"][0]["cells"]["98"]["25"]["content"]

...
"""
Tests for Feature 7 — Writeback: Edit List cells → Update Odoo Records.
...
class TestWriteback(TransactionCase):
    ...
    def _simulate_writeback(
        ...
        try:
            ...
            if model not in env:
                return {"error": "Model %r is not available." % model}

            record = env[model].browse(record_id)
            if not record.exists():
                return {"error": "Record %s(%d) not found." % (model, record_id)}

            try:
                record.check_access("write")
            except AccessError:
                return {"error": "Access denied: you do not have write access on this record."}
    ...
    def test_writeback_access_denied(self):
        ...
        protected_partner = self.env["res.partner"].sudo().create(

...
"""
Tests for spreadsheet.subscription (Feature 8 — Dashboard Subscriptions).
"""
...
class TestSpreadsheetSubscription(TransactionCase):
    ...
    def test_send_digest_with_pivot_data(self):
        """Digest with include_pivot_data=True processes pivot data and updates last_sent."""
    ...
    def test_send_digest_include_false(self):
        """When include_pivot_data=False, pivot rendering is skipped — no <table> in body."""
        self.spreadsheet.write({"spreadsheet_raw": self._make_raw_with_pivot()})
        sub = self._make_subscription(include_pivot_data=False)
    ...
    def test_send_digest_no_email_skips(self):
        """Partner without email: _send_digest() logs a warning and returns without error."""
    ...
    def test_smart_button_count(self):
        ...
        sub1 = self._make_subscription(include_pivot_data=False)
    ...
    def test_action_send_now(self):
        """action_send_now() sends immediately, updating last_sent regardless of due check."""
        sub = self._make_subscription(include_pivot_data=False)
        # Never been sent
        self.assertFalse(sub.last_sent)
        sub.action_send_now()
    ...
    def test_action_open_subscriptions(self):
        ...
        self.assertIn(
            ("spreadsheet_id", "=", self.spreadsheet.id), action["domain"]
        )

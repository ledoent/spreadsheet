...
"""
Tests for spreadsheet.alert (Feature 2 — Threshold Alerts / KPI Watches).
"""
...
class TestSpreadsheetAlert(TransactionCase):
    ...
    def test_read_cell_value_by_sheet_name(self):
        raw = {
            "version": 1,
            "sheets": [
                {"id": "s1", "name": "Summary", "cells": {"2": {"1": {"content": "77"}}}},
                {"id": "s2", "name": "Data", "cells": {"2": {"1": {"content": "99"}}}},
            ],
        }
        self.spreadsheet.write({"spreadsheet_raw": raw})
    ...
    def test_edge_mode_fires_once_on_crossing(self):
        ...
        msg_count_before = self.env["mail.message"].search_count(
            [("res_id", "=", self.spreadsheet.id), ("model", "=", "spreadsheet.spreadsheet")]
        )
        alert._evaluate()
    ...
    def test_level_mode_fires_every_cycle(self):
        ...
        for i in range(3):
            ...
            self.assertGreater(msg_after, msg_before, f"Expected notification on cycle {i+1}")
    ...
    def test_cron_evaluates_all_active(self):
        ...
        alert2 = self._make_alert(name="Low Stock", operator="<", threshold=5.0, cell_ref="B3")
